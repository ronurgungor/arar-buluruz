...
function makeSyntheticPng(): Uint8Array {
  ...
  return concatBytes(PNG_SIGNATURE, pngChunk("IHDR", ihdr), pngChunk("IDAT", new Uint8Array(deflateSync(scanlines))), pngChunk("IEND", new Uint8Array()));
}
...
async function assertNoHorizontalOverflow(page: Page, route: string) {
  ...
  assert(dimensions.documentWidth <= dimensions.viewportWidth, `${route} has horizontal overflow: ${JSON.stringify(dimensions)}`);
}
...
try {
  ...
  assert((await page.getByRole("link", { name: "Giriş" }).count()) === 0, "Operator header exposed a login CTA.");
  ...
  await page.getByLabel("Açıklama", { exact: true }).fill("Kurucu arayüzü ve gerçek private fotoğraf zinciri için sentetik kabul ilanı.");
  ...
  assert((await card.getByText(/1 fotoğraf/).count()) === 1, "Created listing does not have one photo.");
  await card.getByText(/phone · \+1202555/).waitFor();
  const testId = await card.getAttribute("data-testid");
  assert(testId?.startsWith("operator-listing-"), "Operator listing card did not expose its synthetic identity.");
  ...
  await page.locator("li").filter({ hasText: title }).getByRole("button", { name: "Yayınla" }).click();
  ...
  assert(collectionSrc?.includes("/storage/v1/object/sign/listing_photos/"), `Collection did not render signed private photo: ${collectionSrc}`);
  ...
  assert(detailSrc?.includes("/storage/v1/object/sign/listing_photos/"), `Detail did not render signed private photo: ${detailSrc}`);
  ...
  assert((await contactLink.getAttribute("href")) === `tel:${contactE164}`, "Published seller phone drifted.");
  assert((await page.locator('a[href*="wa.me"]').count()) === 0, "Published listing exposed a WhatsApp CTA.");
  await page.screenshot({ path: path.join(resultsDir, "operator-published-detail.png"), fullPage: true });

  await page.goto(`${baseUrl}/kurucu`, { waitUntil: "networkidle" });
  await page.locator("li").filter({ hasText: title }).getByRole("button", { name: "Yayından kaldır" }).click();
  ...
  assert((await page.locator("li").filter({ hasText: title }).count()) === 0, "Hard-deleted listing remained in operator inventory.");

  await page.goto(`${baseUrl}/ara?q=operator`, { waitUntil: "networkidle" });
  assert((await page.getByText(title, { exact: true }).count()) === 0, "Hard-deleted listing remained visible in public search.");
  ...
  assert(sensitiveBrowserMutations.length === 0, `Browser directly performed privileged backend mutations: ${sensitiveBrowserMutations.join(" | ")}`);
