...
if (!(await Bun.file(manifestPath).exists())) throw new Error("pilot-rc finalized manifest is missing.");
...
console.log(`pilot-rc disk manifest JSON.parse passed: ${diskManifestBytes.byteLength} bytes, sha256=${diskManifestSha256}.`);
...
async function readServiceWorkerSnapshot(page: Page): Promise<ServiceWorkerSnapshot> {
  return page.evaluate(async () => {
    const describeWorker = (worker: ServiceWorker | null) => worker ? { scriptURL: worker.scriptURL, state: worker.state } : null;
    ...
    return {
      ...
      cacheNames,
      offlineFallback: fallback ? { status: fallback.status, url: fallback.url, text: await fallback.clone().text() } : null,
    };
  });
}

function hasControllingWorker(snapshot: ServiceWorkerSnapshot) {
  return snapshot.registrations.some((registration) => registration.active?.state === "activated") && snapshot.controller?.state === "activated";
}
...
async function ensureServiceWorkerControl(page: Page) {
  ...
  for (let attempt = 1; attempt <= 30; attempt += 1) {
    ...
    if (hasControllingWorker(snapshot)) {
      console.log(`pilot-rc service worker control diagnostic: reloads=${reloads} state=${JSON.stringify(snapshot)}.`);
      return snapshot;
    }
    if (attempt % 5 === 0) {
...
async function assertManifestAndInstallability(context: BrowserContext, page: Page) {
  ...
  console.log(`pilot-rc served manifest diagnostic: status=${manifestResponse.status()} content-type=${JSON.stringify(contentType)} bytes=${servedManifestBytes.byteLength} sha256=${servedManifestSha256} disk-byte-identical=${byteIdentical}`);
  assert(manifestResponse.ok(), `Manifest HTTP ${manifestResponse.status()}.`);
  assert(contentType.toLocaleLowerCase("en-US").includes("json"), `Manifest content-type is not JSON-compatible: ${contentType}.`);
  const manifest = (await manifestResponse.json()) as Record<string, unknown>;
  assertManifestFields(manifest, "Served manifest");
  assertManifestResidueAbsent(servedManifestText, "Served manifest");
  assert(byteIdentical, `Served manifest differs from finalized disk artifact: disk=${diskManifestSha256} served=${servedManifestSha256}.`);
  const session = await context.newCDPSession(page);
  const result = (await session.send("Page.getInstallabilityErrors")) as { installabilityErrors?: unknown[] };
  assert(Array.isArray(result.installabilityErrors) && result.installabilityErrors.length === 0, `Chromium reported installability errors: ${JSON.stringify(result.installabilityErrors)}`);
}
...
async function assertNoResidue(page: Page, label: string) {
  ...
  assert((await page.locator('a[href*="wa.me"]').count()) === 0, `${label} exposed a WhatsApp CTA.`);
}
...
try {
  ...
  try {
    const desktop = await browser.newContext({ viewport: { width: 1440, height: 900 }, serviceWorkers: "allow" });
    ...
    const operatorPostResponse = await desktop.request.post(`${baseUrl}/kurucu`, { form: { action: "list" }, failOnStatusCode: false });
    assert(operatorPostResponse.status() === 404, `public pilot-rc POST /kurucu returned ${operatorPostResponse.status()}.`);
    ...
    await page.getByText("bu sayfa ad, telefon, ilan metni veya fotoğraf toplamaz", { exact: false }).waitFor();
    ...
    assert((await page.locator("form").count()) === 0, "Pilot intake unexpectedly exposed a data-submission form.");
    assert((await page.locator('input, textarea, input[type="file"]').count()) === 0, "Pilot intake unexpectedly collected browser input.");
    ...
    await page.getByText("Yanlış telefon, size ait veri veya acil kaldırma", { exact: true }).waitFor();
    ...
    const decodedPhoto = await signedPhoto.evaluate(async (image) => {
      ...
      try { await htmlImage.decode(); } catch (error) { decodeError = error instanceof Error ? error.message : String(error); }
      return { complete: htmlImage.complete, naturalWidth: htmlImage.naturalWidth, naturalHeight: htmlImage.naturalHeight, decodeError };
    });
    console.log(`pilot-rc signed photo browser decode diagnostic: complete=${decodedPhoto.complete} naturalWidth=${decodedPhoto.naturalWidth} naturalHeight=${decodedPhoto.naturalHeight} decodeError=${JSON.stringify(decodedPhoto.decodeError)}.`);
    assert(decodedPhoto.naturalWidth > 0, `pilot-rc signed photo did not decode: ${JSON.stringify(decodedPhoto)}.`);
    ...
    assert((await page.locator('a[href*="wa.me"]').count()) === 0, "Pilot detail exposed a WhatsApp seller CTA.");

    const reportHref = await page.getByRole("link", { name: /Yanlış telefon/ }).getAttribute("href");
    ...
    assert((await page.locator("form").count()) === 0, "Pilot takedown route unexpectedly collected report content in-browser.");
    ...
    assert(runtimeErrors.length === 0, `pilot-rc positive desktop runtime errors: ${runtimeErrors.join(" | ")}`);
    ...
    const mobile = await browser.newContext({ viewport: { width: 390, height: 844 }, serviceWorkers: "allow" });
    ...
    const offlineContext = await browser.newContext({ viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true, serviceWorkers: "allow" });
    ...
    offlinePage.on("console", (message) => { if (message.type() === "error") offlineConsoleErrors.push(message.text()); });
    offlinePage.on("pageerror", (error) => offlinePageErrors.push(error.message));
    offlinePage.on("requestfailed", (request) => offlineRequestFailures.push(`${request.method()} ${request.url()} ${request.failure()?.errorText ?? "[unknown]"}`));
    offlinePage.on("response", (response) => {
      if (response.request().resourceType() === "document") offlineDocumentResponses.push(`${response.status()} ${response.url()} fromServiceWorker=${response.fromServiceWorker()}`);
    });
    ...
    try {
      ...
      assert(initialSnapshot.registrations.length === 1, `pilot-rc offline proof saw unexpected service worker registrations: ${JSON.stringify(initialSnapshot.registrations)}.`);
      assert(initialSnapshot.offlineFallback?.status === 200 && initialSnapshot.offlineFallback.text.includes("<h1>Bağlantı yok</h1>"), `pilot-rc service worker cache did not contain the fail-closed offline fallback: ${JSON.stringify(initialSnapshot.offlineFallback)}.`);
      const beforeOutageSnapshot = await ensureServiceWorkerControl(offlinePage);
      assert(hasControllingWorker(beforeOutageSnapshot), `pilot-rc lost service worker control before the real offline navigation: ${JSON.stringify(beforeOutageSnapshot)}.`);
      assert(beforeOutageSnapshot.offlineFallback?.text.includes("<h1>Bağlantı yok</h1>") === true, "pilot-rc offline fallback disappeared from Cache Storage before the outage.");
      ...
      try {
        ...
        await offlinePage.evaluate((url) => { window.location.assign(url); }, offlineUrl);
      ...
      assert(offlineNavigationError === null && offlineNavigationResponse !== null, `pilot-rc real offline navigation failed before the service worker fallback could respond: ${offlineNavigationError}.`);
      assert(offlinePage.url() === offlineUrl, `pilot-rc offline navigation landed on an unexpected URL: ${offlinePage.url()}.`);
      assert(offlineNavigationResponse.fromServiceWorker(), `pilot-rc offline navigation was not fulfilled by the controlling service worker: ${offlineNavigationResponse.url()}.`);
      assert(offlineNavigationResponse.ok(), `pilot-rc service worker offline fallback returned HTTP ${offlineNavigationResponse.status()}.`);
      const offlineResponseText = await offlineNavigationResponse.text();
      assert(offlineResponseText === beforeOutageSnapshot.offlineFallback?.text, "pilot-rc service worker navigation response differed from the precached /offline.html response.");
      const afterNavigationSnapshot = await readServiceWorkerSnapshot(offlinePage);
      console.log(`pilot-rc real offline navigation diagnostic: response=${offlineNavigationResponse.status()} ${offlineNavigationResponse.url()} fromServiceWorker=${offlineNavigationResponse.fromServiceWorker()} error=${JSON.stringify(offlineNavigationError)} pageUrl=${offlinePage.url()} state=${JSON.stringify(afterNavigationSnapshot)} documentResponses=${JSON.stringify(offlineDocumentResponses)} requestFailures=${JSON.stringify(offlineRequestFailures)} consoleErrors=${JSON.stringify(offlineConsoleErrors)} pageErrors=${JSON.stringify(offlinePageErrors)}.`);
      await offlinePage.getByRole("heading", { level: 1, name: "Bağlantı yok" }).waitFor();
      await offlinePage.getByText("dinamik ilanları çevrimdışı saklamaz", { exact: false }).waitFor();
      assert((await offlinePage.getByText(baselineTitle, { exact: true }).count()) === 0, "Offline fallback exposed stale dynamic listing data.");
      assert(offlineConsoleErrors.length === 0 && offlinePageErrors.length === 0, `pilot-rc offline fallback emitted runtime errors: console=${offlineConsoleErrors.join(" | ")} page=${offlinePageErrors.join(" | ")}`);
      await assertNoHorizontalOverflow(offlinePage, "offline fallback mobile");
      await offlinePage.screenshot({ path: path.join(resultsDir, "pilot-rc-mobile-offline.png"), fullPage: true });
    } finally {
      await offlineContext.setOffline(false).catch(() => undefined);
      await offlineContext.close();
    }

    console.log("pilot-rc activation-facing desktop/mobile/PWA/offline/navigation/fail-closed proof passed.");
  } finally {
    await browser.close();
  }
} finally {
