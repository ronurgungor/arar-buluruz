...
- same-facet values are OR;
- different facets are AND;
- a listing missing a structured value fails an active contextual filter;
...
- no numeric slider is required; price uses explicit min/max inputs.
...
The UI metadata source is `src/lib/product-finding-ui-contract.ts`.
...
- contextual facet groups are capped at five per product type by contract;
- no desktop control-panel layout, no Advanced Filters junk drawer, no giant taxonomy;
- contextual filter options are derived from currently loaded validated listing values rather than a second hand-maintained taxonomy;
