            ...
            <ul className="mt-2 divide-y divide-border/70">
              {results.map((listing, index) => (
                <Fragment key={listing.id}>
                  ...
                  {index === 2 && results.length >= 4 ? <AdSlot placement="search_infeed_1" container="li" className="py-3" /> : null}
                </Fragment>
              ))}
            </ul>
            {results.length === 0 && (
              <div className="mt-10 text-center">
                ...
                {activeCity !== ALL_CITIES && (
                  <button type="button" onClick={() => setSearch({ il: ALL_CITIES, ilce: ALL_DISTRICTS })} className="mt-4 h-11 rounded-full bg-primary px-6 text-sm font-bold text-primary-foreground hover:bg-primary/90">Tüm Türkiye'de ara</button>
                )}
              </div>
            )}
          </>
