...
import { ArrowDown, ArrowUp, CheckCircle2, ChevronLeft, ChevronRight, ImagePlus, X } from "lucide-react";
...
export const Route = createFileRoute("/ilan-ver")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { handleStage1SelfServiceRequest } = await import("../lib/stage1-self-service-server");
...
function Stage1ListingWizard() {
  ...
  const addPhotos = (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (next.reduce((sum, item) => sum + item.file.size, 0) + file.size > STAGE1_MAX_TOTAL_UPLOAD_BYTES) {
  ...
  const submitWithCapability = async (capability: string) => {
  ...
  const beginFinalSubmit = async () => {
    ...
    if (cachedCapability) {
      const submitted = await submitWithCapability(cachedCapability);
      if (submitted || error) {
        setBusy(false);
        return;
      }
      ...
      <div className="min-h-screen">
        ...
        <main className="mx-auto max-w-xl px-4 pb-16">
          <section className="mt-10 rounded-2xl border border-border bg-card p-6 text-center">
            ...
            <h1 className="mt-4 text-2xl font-extrabold tracking-tight">İlanınız incelemeye alındı</h1>
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-xl px-4 pb-20">
        ...
        {step === 1 && (
          <section className="mt-6">
            ...
            <label
              ...
              className="mt-4 flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card px-4 text-center hover:bg-accent/40"
            >
              <ImagePlus aria-hidden className="h-7 w-7 text-primary" />
              <span className="mt-2 text-sm font-bold">Fotoğraf ekle</span>
              <span className="mt-1 text-xs text-muted-foreground">JPEG, PNG veya WebP · dosya başına 8 MB</span>
            </label>
            {photos.length > 0 && (
              <ol className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                {photos.map((photo, index) => (
                  <li key={photo.id} className="overflow-hidden rounded-xl border border-border bg-card">
                    <img src={photo.previewUrl} alt={`Seçilen fotoğraf ${index + 1}`} className="aspect-[4/3] w-full object-cover" />
                    <div className="p-2">
                      <div className="flex items-center justify-between gap-2">
                        <span className="truncate text-xs font-semibold">{index === 0 ? "Kapak" : `${index + 1}. fotoğraf`}</span>
                        <button type="button" onClick={() => removePhoto(photo.id)} aria-label={`${index + 1}. fotoğrafı kaldır`} className="rounded-full p-2 hover:bg-accent">
                          <X aria-hidden className="h-4 w-4" />
                        </button>
                      </div>
                      <div className="mt-1 flex gap-1">
                        <button type="button" disabled={index === 0} onClick={() => movePhoto(index, -1)} aria-label="Fotoğrafı öne al" className="min-h-9 flex-1 rounded-lg border border-border disabled:opacity-30">
                          <ArrowUp aria-hidden className="mx-auto h-4 w-4" />
                        </button>
                        <button type="button" disabled={index === photos.length - 1} onClick={() => movePhoto(index, 1)} aria-label="Fotoğrafı geriye al" className="min-h-9 flex-1 rounded-lg border border-border disabled:opacity-30">
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              {photos.length}/{STAGE1_MAX_PHOTOS} fotoğraf · {(totalPhotoBytes / 1024 / 1024).toFixed(1)} MB
            </p>
          </section>
        )}
        ...
        {step === 2 && (
          <section className="mt-6 space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Alıcının ürünü hızlıca anlaması için temel bilgiler yeterli.</p>
            </div>
            <label className="block">
              <span className="text-sm font-medium">Kategori</span>
              <select value={category} onChange={(event) => setCategory(event.target.value as Stage1Category)} className={`mt-1 ${selectClass}`}>
                {STAGE1_CATEGORIES.map((value) => <option key={value} value={value}>{STAGE1_CATEGORY_LABELS[value]}</option>)}
              </select>
            </label>
            <label className="block">
              <span className="text-sm font-medium">Başlık</span>
              <input value={title} onChange={(event) => setTitle(event.target.value)} minLength={3} maxLength={120} placeholder="Örn. IKEA çalışma masası" className={`mt-1 ${fieldClass}`} />
            </label>
            <label className="block">
              <span className="text-sm font-medium">Durum</span>
              <select value={condition} onChange={(event) => setCondition(event.target.value as Stage1Condition)} className={`mt-1 ${selectClass}`}>
                {STAGE1_CONDITIONS.map((value) => <option key={value} value={value}>{STAGE1_CONDITION_LABELS[value]}</option>)}
              </select>
            </label>
            <label className="block">
              <span className="text-sm font-medium">Fiyat (TL)</span>
              <input disabled={isFree} value={price} onChange={(event) => setPrice(event.target.value)} inputMode="decimal" placeholder="0" className={`mt-1 ${fieldClass} disabled:bg-muted disabled:text-muted-foreground`} />
            </label>
        ...
        {step === 3 && (
          <section className="mt-6 space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Ürünün durumu ve önemli ayrıntıları yazın. Açık adres istemiyoruz.</p>
            </div>
            <label className="block">
              <span className="text-sm font-medium">Açıklama</span>
              <textarea value={description} onChange={(event) => setDescription(event.target.value)} minLength={10} maxLength={5000} rows={6} placeholder="Ürünün durumu, ölçüsü, varsa kusurları..." className="mt-1 w-full rounded-xl border border-border bg-card p-4 text-base outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10" />
            </label>
            ...
            <p className="text-xs text-muted-foreground">Stage 1 yalnız Çorlu ilanlarını kabul eder; tam ev adresi alınmaz.</p>
          </section>
        )}
        ...
        {step === 4 && (
          <section className="mt-6 space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">İlan onaylandığında alıcılar seçtiğiniz yöntemle doğrudan size ulaşır.</p>
            </div>
            <div className="rounded-xl border border-border bg-accent/35 p-3 text-sm leading-relaxed text-muted-foreground">
              İlan oluşturmak için verdiğiniz bilgiler moderasyon amacıyla işlenir. Telefonunuz yalnız ilan onaylanırsa seçtiğiniz iletişim yöntemlerinde görünür. <Link to="/gizlilik" className="font-semibold text-primary underline underline-offset-4">Gizlilik ve Aydınlatma</Link>
            </div>
            <label className="block">
              <span className="text-sm font-medium">İlanda görünecek ad</span>
              <input value={sellerDisplayName} onChange={(event) => setSellerDisplayName(event.target.value)} minLength={2} maxLength={80} autoComplete="name" placeholder="Adınız" className={`mt-1 ${fieldClass}`} />
            </label>
            <label className="block">
              <span className="text-sm font-medium">Telefon</span>
              <input value={phone} onChange={(event) => { setPhone(event.target.value); setVerificationChallengeId(null); }} inputMode="tel" autoComplete="tel" placeholder="+905xxxxxxxxx" className={`mt-1 ${fieldClass}`} />
              <span className="mt-1 block text-xs text-muted-foreground">Gönderirken telefon kontrolü yapılır; klasik hesap oluşturmanız gerekmez.</span>
            </label>
            <fieldset>
              <legend className="text-sm font-medium">İletişim tercihi</legend>
              <div className="mt-2 grid gap-2 sm:grid-cols-3">
                {STAGE1_CONTACT_PREFERENCES.map((value) => (
                  <label key={value} className={`flex min-h-12 cursor-pointer items-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold ${contactPreference === value ? "border-primary bg-primary/5" : "border-border bg-card"}`}>
            ...
            <div className="space-y-3 rounded-xl border border-border bg-card p-4 text-sm">
              <label className="flex items-start gap-3">
                <input type="checkbox" checked={privateSellerConfirmed} onChange={(event) => setPrivateSellerConfirmed(event.target.checked)} className="mt-1" />
                <span>Özel kişi olarak ara sıra ilan veriyorum; ürün kendi kullanılmış eşyamdır.</span>
              </label>
              <label className="flex items-start gap-3">
                <input type="checkbox" checked={contentRightsConfirmed} onChange={(event) => setContentRightsConfirmed(event.target.checked)} className="mt-1" />
                <span>Fotoğraf ve metni paylaşmaya yetkim var; çocuk/üçüncü kişi veya hassas kişisel veri eklemedim.</span>
              </label>
              <label className="flex items-start gap-3">
                ...
                <span>Bu telefon bana ait; ilan yayınlanırsa seçtiğim iletişim yöntemleri için kamuya açık gösterilmesini istiyorum.</span>
              </label>
            </div>
            <div className="rounded-xl border border-border bg-muted/30 p-4 text-sm">
              ...
              <p className="mt-1 text-xs text-muted-foreground">{PILOT_PROVINCE} / {PILOT_DISTRICT} · {STAGE1_CONTACT_LABELS[contactPreference]}</p>
            </div>
            {verificationChallengeId && (
              <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">
                <label className="block">
                  <span className="text-sm font-semibold">6 haneli doğrulama kodu</span>
                  <input value={verificationCode} onChange={(event) => setVerificationCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" placeholder="000000" className={`mt-2 ${fieldClass}`} />
                </label>
                <button type="button" disabled={busy} onClick={() => void confirmVerification()} className="mt-3 h-12 w-full rounded-full bg-primary px-5 text-sm font-bold text-primary-foreground disabled:opacity-50">
                  {busy ? "Doğrulanıyor…" : "Doğrula ve gönder"}
                </button>
              </div>
            )}
            <p className="text-xs text-muted-foreground">
              İlan Kuralları'nı okuyabilirsiniz: <Link to="/ilan-kurallari" className="font-semibold text-primary underline underline-offset-4">İlan Kuralları</Link>
            </p>
          </section>
        )}
        ...
        <div className="mt-8 flex items-center gap-3">
          {step > 1 && (
            <button type="button" disabled={busy} onClick={() => { clearError(); setVerificationChallengeId(null); setStep((current) => current - 1); }} className="inline-flex h-12 min-w-12 items-center justify-center gap-1 rounded-full border border-border px-4 text-sm font-semibold disabled:opacity-50">
              <ChevronLeft aria-hidden className="h-4 w-4" /> Geri
            </button>
          )}
          {step < 4 ? (
            <button type="button" onClick={() => { if (validateCurrentStep()) setStep((current) => current + 1); }} className="ml-auto inline-flex h-12 flex-1 items-center justify-center gap-1 rounded-full bg-primary px-5 text-sm font-bold text-primary-foreground">
              Devam <ChevronRight aria-hidden className="h-4 w-4" />
            </button>
          ) : !verificationChallengeId ? (
            <button type="button" disabled={busy} onClick={() => void beginFinalSubmit()} className="ml-auto h-12 flex-1 rounded-full bg-primary px-5 text-sm font-bold text-primary-foreground disabled:opacity-50">
