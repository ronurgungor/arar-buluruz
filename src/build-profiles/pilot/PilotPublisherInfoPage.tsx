...
type PilotPublisherInfoPageProps = {
  title: string;
  intro: string;
  sections: readonly PublisherSection[];
};

export function PilotPublisherInfoPage({ title, intro, sections }: PilotPublisherInfoPageProps) {
  return (
      ...
      <main className="mx-auto max-w-2xl px-4 pb-16 pt-8">
        <article className="rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-10">
          ...
          <p className="mt-10 border-t border-border pt-6 text-sm leading-6 text-muted-foreground">
            Bu sayfa Arar Buluruz'un mevcut çalışma ve güvenlik sınırlarını açıklar.
          </p>
          <Link
            to="/"
