...
function assertLocalSameOriginRequest(request: Request): void {
  ...
  if (!isLoopbackHost(url.hostname)) {
    throw new ModerationError("LOCAL_ONLY", "Kurucu moderasyon yüzeyi yalnız yerel bağlantıda çalışır.");
  }
...
function readConfig(): BackendConfig {
  ...
  if (url.protocol !== "https:" && !(isLocal && url.protocol === "http:")) {
    throw new ModerationError("BACKEND_UNAVAILABLE", "Kurucu backend yerel test dışında HTTPS kullanmalıdır.");
  }
  for (const ref of TARLADAN_PROJECT_REFS) {
    if (url.hostname.includes(ref)) {
      throw new ModerationError("BACKEND_UNAVAILABLE", "Tarladan kurucu moderasyonunda kapsam dışıdır.");
    }
  }
...
async function fetchListing(config: BackendConfig, id: string): Promise<BackendRow | null> {
  ...
  url.searchParams.set("select", "id,title,description,price_amount,price_is_free,category,item_condition,seller_display_name,status,contact_channel,contact_e164,contact_verified_at,publication_instruction_at,created_at,published_at,expires_at,unpublished_at");
  url.searchParams.set("limit", "1");
  const response = await requireOk(await fetch(url, { headers: { ...headers(config), Accept: "application/json" } }), "moderation listing read");
...
async function fetchPhotos(config: BackendConfig, listingId: string): Promise<PhotoRow[]> {
  ...
  if (!Array.isArray(rows) || rows.length > MAX_PHOTOS) throw new Error("Invalid moderation photo inventory.");
  return rows;
}
...
async function signPhoto(config: BackendConfig, objectPath: string): Promise<string> {
  const response = await requireOk(
    await fetch(`${config.baseUrl}/storage/v1/object/sign/listing_photos/${encodeObjectPath(objectPath)}`, {
      method: "POST",
      headers: headers(config),
...
async function listListings(config: BackendConfig): Promise<Stage1ModerationListing[]> {
  ...
  const response = await requireOk(await fetch(url, { headers: { ...headers(config), Accept: "application/json" } }), "moderation listing inventory");
  const rows = (await response.json()) as BackendRow[];
  return await Promise.all(rows.map(async (row) => {
    if (!UUID_PATTERN.test(row.id) || !isPilotListingStatus(row.status)) throw new Error("Moderation row violated identity/status contract.");
    const photoRows = await fetchPhotos(config, row.id);
    const photoUrls = await Promise.all(photoRows.map((photo) => signPhoto(config, photo.object_path)));
    return {
      id: row.id,
      title: row.title,
...
async function patchListing(config: BackendConfig, id: string, patch: Record<string, unknown>, context: string): Promise<void> {
  const url = new URL(`${config.baseUrl}/rest/v1/listings`);
  url.searchParams.set("id", `eq.${id}`);
  const response = await requireOk(
    await fetch(url, {
  ...
  if (rows.length !== 1 || rows[0]?.id !== id) throw new Error(`${context} did not mutate exactly one listing.`);
}
...
async function publish(config: BackendConfig, form: FormData): Promise<string> {
  ...
  if (!listing || (listing.status !== "pending" && listing.status !== "unpublished")) {
    throw new ModerationError("INVALID_STATE", "Yalnız incelemedeki veya yayından kaldırılmış ilan yayınlanabilir.");
  }
  ...
  if (!Number.isInteger(days) || days < 1 || days > 90) throw new ModerationError("INVALID_REQUEST", "Yayın süresi 1–90 gün olmalıdır.");
  const now = new Date();
  await patchListing(config, id, {
    status: "published",
...
async function reject(config: BackendConfig, form: FormData): Promise<string> {
  const id = requiredUuid(form);
  const listing = await fetchListing(config, id);
  if (!listing || listing.status !== "pending") throw new ModerationError("INVALID_STATE", "Yalnız incelemedeki ilan reddedilebilir.");
...
async function unpublish(config: BackendConfig, form: FormData): Promise<string> {
  const id = requiredUuid(form);
  const listing = await fetchListing(config, id);
  if (!listing || listing.status !== "published") throw new ModerationError("INVALID_STATE", "Yalnız yayındaki ilan kaldırılabilir.");
  await patchListing(config, id, { status: "unpublished", unpublished_at: new Date().toISOString() }, "moderation unpublish");
  return id;
}
...
async function hardDelete(config: BackendConfig, form: FormData): Promise<string> {
  ...
  if (listing.status === "published") {
    await patchListing(config, id, { status: "unpublished", unpublished_at: new Date().toISOString() }, "moderation pre-delete unpublish");
    listing = await fetchListing(config, id);
    if (!listing || listing.status !== "unpublished") throw new Error("Listing did not fail closed before delete.");
  }
  ...
  const url = new URL(`${config.baseUrl}/rest/v1/listings`);
  url.searchParams.set("id", `eq.${id}`);
  await requireOk(await fetch(url, { method: "DELETE", headers: headers(config) }), "moderation listing delete");
  if (await fetchListing(config, id)) throw new Error("Listing row remained after delete.");
  if ((await fetchPhotos(config, id)).length !== 0) throw new Error("Photo metadata remained after delete.");
  return id;
}
...
export async function handleStage1ModerationRequest(request: Request): Promise<Response> {
  try {
    ...
    if (action === "list") return jsonResponse({ ok: true, message: "Moderasyon kuyruğu güncellendi.", listings: await listListings(config) });
    if (action === "publish") return jsonResponse({ ok: true, message: "İlan yayınlandı.", listingId: await publish(config, form) });
    if (action === "reject") return jsonResponse({ ok: true, message: "İlan reddedildi.", listingId: await reject(config, form) });
    if (action === "unpublish") return jsonResponse({ ok: true, message: "İlan yayından kaldırıldı.", listingId: await unpublish(config, form) });
    if (action === "delete") return jsonResponse({ ok: true, message: "İlan ve ilişkili fotoğraflar silindi.", listingId: await hardDelete(config, form) });
    ...
    return jsonResponse({ ok: false, code: "OPERATION_FAILED", message: "Moderasyon işlemi güvenli biçimde tamamlanamadı." }, 500);
  }
}
