...
const baseRow = {
  ...
  description: "Synthetic listing proving the capability-gated detail contact payload.",
  price_amount: "1250.00",
  price_is_free: false,
  category: "home",
  product_type: null,
  product_attributes_version: null,
  product_attributes: {},
  item_condition: "good",
  province: "Tekirdağ",
  district: "Çorlu",
...
function contactBoundaryFetch(
  ...
  return (async (input, init) => {
    ...
    if (url.pathname === "/rest/v1/rpc/get_public_listing_contact") {
      expect(init?.method).toBe("POST");
      expect(JSON.parse(String(init?.body))).toEqual({ p_listing_id: baseRow.id });
      return new Response(JSON.stringify(contactRows), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
...
describe("public listings seller-contact capability boundary", () => {
  test("collection query and mapped card payload omit seller contact", async () => {
    let requestedUrl: URL | undefined;

    const listings = await fetchPublicListings(
      config,
      contactBoundaryFetch(publicContactRows, (url) => {
        requestedUrl = url;
      }),
    );
  ...
  test("detail keeps contact columns out of direct REST and obtains contact through the capability RPC", async () => {
    let requestedUrl: URL | undefined;
  ...
  test("contact suppression returns a visible detail with no public contact or CTA source", async () => {
    const listing = await fetchPublicListing(
      baseRow.id,
      config,
      contactBoundaryFetch([], () => {}),
    );
    expect(listing).not.toBeNull();
    expect(listing?.publicContact).toBeNull();
  });

  test("fails closed if the capability RPC returns malformed contact evidence", async () => {
    await expect(
      fetchPublicListing(
        baseRow.id,
        config,
        contactBoundaryFetch([{ contact_channel: "email", contact_e164: "bad" }], () => {}),
      ),
    ).rejects.toBeInstanceOf(PublicListingsError);
  });
